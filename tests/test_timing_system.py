...
class TestETFAllocation:
    ...
    def test_no_target_uses_etf_list(self):
        ...
        etfs = [
            SimpleNamespace(code="510880", name="AA中证红利ETF", index_code="000922", index_name="中证红利指数"),
            SimpleNamespace(code="516828", name="BB红利低波ETF", index_code="930846", index_name="中证红利低波动指数"),
        ]
        result = TimingSystem.suggest_etf_allocation(0.8, etf_list=etfs)
        assert len(result) == 2
        # 000922(中证红利) → 0.5 * 0.8 = 0.4, 930846(红利低波) → 0.3 * 0.8 = 0.24
        weights = [r["权重"] for r in result]
        assert 0.4 in weights
        assert 0.24 in weights
