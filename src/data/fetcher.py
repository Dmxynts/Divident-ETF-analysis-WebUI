...
class DataFetcher:
    ...
    # -----------------------------------------------------------
    # 缓存管理（使用 parquet 格式，避免 pickle 的安全风险）
    # -----------------------------------------------------------
    ...
    def _load_cache(self, name: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not path.exists():
                # 兼容旧版 pickle 缓存，自动迁移
                old_path = _get_cache_dir() / f"{name}.pkl"
                if old_path.exists():
                    with open(old_path, "rb") as f:
                        obj = pickle.load(f)
                    self._save_cache(name, obj["data"])
                    old_path.unlink(missing_ok=True)
                    return obj["data"]
                return None
            # 检查过期（优先用 sidecar 元数据，其次文件 mtime）
            meta_path = path.with_suffix(".meta.json")
            if meta_path.exists():
                with open(meta_path, "r") as f:
                    meta = json.load(f)
                saved_time = datetime.fromisoformat(meta["time"])
                if (datetime.now() - saved_time).days > self.cache_expire_days:
                    return None
            else:
    ...
    def get_cache_info(self) -> dict:
        ...
        for cat_name, prefix in self.CACHE_CATEGORIES.items():
            files = [f for f in cache_dir.glob(f"{prefix}*.parquet")]
            if files:
                info[cat_name] = {}
        ...
        # 不在分类中的也列出来
        all_parquet = set(cache_dir.glob("*.parquet"))
        categorized = set()
        for prefix in self.CACHE_CATEGORIES.values():
            categorized.update(cache_dir.glob(f"{prefix}*.parquet"))
        other = sorted(all_parquet - categorized)
        if other:
            info["其他"] = {f.name: datetime.fromtimestamp(f.stat().st_mtime).strftime("%Y-%m-%d %H:%M") for f in other}
        return info
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    continue
                raise ConnectionError(f"数据获取失败({func.__name__}): {e}") from e
        return None
    ...
    def _normalize_price_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """统一标准化价格数据列名（保留 close/high/low/date）"""
        rename_map = {}
