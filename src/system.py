...
class DividendETFQuantSystem:
    ...
    def __init__(self):
        ...
        # ETF映射表（用于数据回退）
        etf_index_map = {etf.code: etf.index_code for etf in CFG.etfs}
        self._etf_list = CFG.etfs
    ...
    # 模块1: 股债利差择时
    # -----------------------------------------------------------
    def run_spread_timing(self, index_code: str = CFG.etfs[0].index_code,
    ...
    # 模块4: 风险管理
    # -----------------------------------------------------------
    def run_risk_analysis(self, etf_code: str = CFG.etfs[0].code,
        ...
        return {"risk_report": report, "distribution": dist, "evt": evt_result,
                "returns": returns, "etf_data": etf_data,
                "dd_info": dd_info, "sl_info": sl_info,
                "scenarios": scenarios, "budget": budget}
    ...
    # 模块6: 因子归因
    # -----------------------------------------------------------
    def run_factor_attribution(self, etf_code: str = CFG.etfs[0].code, years: int = 5):
    ...
    # 模块7: 综合择时系统
    # -----------------------------------------------------------
    def run_comprehensive_timing(self, index_code: str = CFG.etfs[0].index_code,
                                  etf_code: str = CFG.etfs[0].code):
        """运行综合择时系统"""
        print_header("综合择时系统")
    ...
    # 一键全运行
    # -----------------------------------------------------------
    def run_all(self, index_code: str = CFG.etfs[0].index_code, etf_code: str = CFG.etfs[0].code,
                 plot: bool = False):
        """运行全部分析模块"""
        logger.info("=" * 55)
