...
# 运行时 ETF 列表由 CFG.etfs 提供，不再使用模块级可变全局变量
...
class Config:
    ...
    def __init__(self):
        ...
        # 有效 ETF 列表（默认从 _DEFAULT_ETFS 复制，可被 YAML 覆盖）
        self.etfs: List[ETFConfig] = list(_DEFAULT_ETFS)
    ...
    def index_codes(self) -> List[str]:
        return list(set(etf.index_code for etf in self.etfs))
